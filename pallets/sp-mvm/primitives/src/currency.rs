...
use sp_std::{cmp::PartialEq, convert::TryFrom, default::Default, vec::Vec};
...
#[cfg(test)]
mod tests {
	use super::{CurrencyId, TryFrom, NATIVE_SYM};
	...
	fn try_from_vec() {
		assert_eq!(CurrencyId::try_from(NATIVE_SYM.to_vec()).unwrap(), CurrencyId::NATIVE);
	...
	/// Test try from &[u8].
	fn try_from_slice() {
		assert_eq!(CurrencyId::try_from(NATIVE_SYM.as_ref()).unwrap(), CurrencyId::NATIVE);
		assert_eq!(CurrencyId::try_from(b"KSM".as_ref()).unwrap(), CurrencyId::KSM);
