...
//! Converts Move VM execution results (Status, Gas Limit, Errors) to Substrate compatible results
//! (Dispatch Results).
//!
//! So in a nutshell, after getting the execution result from Move VM we should convert it to a
//! format compatible with Substrate (usually it's DispatchResultWithPostInfo). At the same time we
//! convert status codes to Substrate errors if there is error. Also, gas would be converted to
//! weight and back here.
use super::{Config, Error};
use crate::gas::GasWeightMapping;
use frame_support::dispatch::{
	DispatchErrorWithPostInfo, DispatchResultWithPostInfo, Pays, PostDispatchInfo, Weight,
};
...
pub fn from_status_code_with_gas<T: Config>(
	...
	let gas = PostDispatchInfo { actual_weight: Some(gas), pays_fee: Pays::Yes };

	match code {
		StatusCode::EXECUTED => Ok(gas),
		_ => Err({
			DispatchErrorWithPostInfo { post_info: gas, error: Error::<T>::from(code).into() }
		}),
	}
}
...
///
/// VM returns the VM result, so we use the current function to convert it to
/// DispatchResultWithPostInfo.
