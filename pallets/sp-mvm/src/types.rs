...
use move_core_types::{
	...
	language_storage::{
		ModuleId as InternalModuleId, StructTag as InternalStructTag, TypeTag as InternalTypeTag,
	},
};
...
impl<AccountId: DecodeT> TryFrom<InternalModuleId> for MoveModuleId<AccountId> {
	...
	/// Convert InternalModuleId (that's used by Move VM internally) to MoveModuleId that can be
	/// used outside.
...
impl<AccountId: DecodeT> TryFrom<InternalTypeTag> for MoveTypeTag<AccountId> {
	...
	/// Convert InternalTypeTag (that's used by Move VM internally) to MoveTypeTag that can be used
	/// outside.
...
impl<AccountId: DecodeT> MoveStructTag<AccountId> {
	...
	pub fn new(owner: AccountId, module: Identifier, name: Identifier, ty_params: Vec<()>) -> Self {
		Self {
			owner,
...
impl<AccountId: DecodeT> TryFrom<InternalStructTag> for MoveStructTag<AccountId> {
	...
	/// Converts InternalStructTag (that's used by Move VM internally) to MoveStructTag that can be
	/// used outside.
