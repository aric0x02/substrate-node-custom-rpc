...
//! This pallet enables Move Virtual Machine developed by Facebook's Diem team in your Substrate
//! based chain. The pallet allows to execute Move Smart contracts, utilizing Move VM adopted for
//! WASM Runtime. You can find Move VM in the following repository - https://github.com/pontem-network/sp-move-vm

//! Move VM has two types of smart contracts: scripts and modules.
//! Modules can be stored in the chain storage, under user account (address) and module name,
//! modules code that can be resused multiplay times. Also, modules store user data, similar to EVM
//! smart contracts. Scripts executed only one time, during transaction execution, it can access
//! modules, or not (depends on script goal). Scripts used as entry point, because indeed scripts
//! launch code execution in Move VM. Read more about scripts and modules in Pontem documentation - https://docs.pontem.network/03.-move-vm/modules
...
//! publish_module(module_bc: Vec<u8>, gas_limit: u64) - publish Move module with bytecode
//! `module_bc`. publish_package(package: Vec<u8>, gas_limit: u64) - publish package (a set of Move
//! modules) from binary `package`.
...
pub mod pallet {
	...
	use crate::{
		balance::boxed::BalancesAdapter, storage::boxed::VmStorageBoxAdapter as StorageAdapter,
	};
	...
	use support::{
		...
		pallet_prelude::*,
		traits::{tokens::fungibles, UnixTime},
		PalletId,
	};
	...
	use move_core_types::{
		...
		language_storage::{ModuleId as InternalModuleId, CORE_CODE_ADDRESS},
	};
	use move_vm::{
		abi::ModuleAbi,
		io::context::ExecutionContext,
		mvm::Mvm,
		types::{Gas, ModulePackage, ModuleTx, Transaction, VmResult},
		StateAccess, Vm,
	};
	#[cfg(not(feature = "std"))]
	extern crate alloc;
		...
		frame_system::Config + timestamp::Config + balances::Config + groupsign::Config
	{
		...
		/// The AccountId that can perform a standard library update or deploy module under 0x
		/// address.
		...
		OriginFor<T>: Into<Result<groupsign::Origin<T>, OriginFor<T>>>,
	{
		...
		/// Publish module package (could be generated using 'dove build -b'), e.g.: several modules
		/// in one transaction.
		///
		/// Deploy several modules in one transaction. Could be called by root in case needs to
		/// update Standard Library. Read more about Standard Library - https://docs.pontem.network/03.-move-vm/stdlib
		/// The gas limit should be provided.
	...
	/// Accepts Standard Library modules list and initialize state by calling initialize function
	/// with arguments.
	...
	/// Move VM allows us to configure Gas Price, but we use constant for gas price, as we follow
	/// general Substrate approach with weight and tips.
	...
	impl<T: Config> Pallet<T> {
		...
		/// Execute Move VM script with provided signers, script byte code, gas limit, and dry run
		/// configuration. In case of dry run nothing would be written to storage after execution
		/// (required mostly by RPC calls, e.g. estimate gas etc). Multiple signers supported by
		/// utilizing multisig pallet.
			...
			T::BlockNumber: TryInto<u64>,
		{
			...
			let tx = {
				let signers = if transaction.signers_count() == 0 { &[] } else { signers };
				...
				let signers =
					signers.iter().map(addr::account_to_bytes).map(AccountAddress::new).collect();
		...
		pub fn ensure_and_convert(
			...
			match T::UpdateOrigin::ensure_origin(origin.clone()) {
				...
				Err(_) => {
					let signer = ensure_signed(origin).map_err(|_| Error::<T>::InvalidSignature)?;
					Ok((addr::account_to_account_address(&signer), signer))
				},
			}
		}

		/// Publish Move module script with provided account, module bytecode, gas limit, and dry
		/// run configuration. In case of dry run nothing would be written to storage after
		/// execution (required mostly by RPC calls, e.g. estimate gas etc).
		...
		fn move_module_id_to_module_id(
			owner: &T::AccountId,
			module: Vec<u8>,
		...
		fn get_abi(owner: &T::AccountId, module: Vec<u8>) -> Result<Option<Vec<u8>>, Vec<u8>> {
			let abi = ModuleAbi::from(
				CompiledModule::deserialize(
					&Self::get_module_abi_by_address_and_name(owner, module)?.unwrap(),
				)
				.unwrap(),
			);
			ensure!(!abi.funcs.is_empty(), vec![]);

			debug!("abi result: {:?}", abi);
			Ok(Some(vec![]))
		}
		pub fn get_module_abi_by_address_and_name(
			owner: &T::AccountId,
			module: Vec<u8>,
		) -> Result<Option<Vec<u8>>, Vec<u8>> {
		...
		pub fn get_module_by_address_and_name(
			owner: &T::AccountId,
			module: Vec<u8>,
		) -> Result<Option<Vec<u8>>, Vec<u8>> {
			if let Ok(id) = Self::move_module_id_to_module_id(owner, module) {
	...
	///
	/// Supports both static (created at launch of chain), and dynamic one (usually we use regulated
	/// one);.
	...
	pub enum Error<T> {
		...
		/// This write set transaction was rejected because it did not meet the requirements for
		/// one.
		RejectedWriteSet,
		...
		/// Max gas units submitted with transaction not enough to cover the intrinsic cost of the
		/// transaction.
		...
		/// Gas unit price submitted with the transaction is above the maximum gas price set in the
		/// VM.
		GasUnitPriceAboveMaxBound,
		/// Gas specifier submitted is either malformed (not a valid identifier), or does not refer
		/// to an accepted gas specifier
		InvalidGasSpecifier,
		...
		/// The module does not have any module handles. Each module or script must have at least
		/// one module handle.
		NoModuleHandles,
