...
//! To be in compatibility with Substrate SS58 addresses we implemented traits and functions that
//! allow us to convert SS58 addresses to 0x{hex} format. Current file contains traits and functions
//! which allows us to convert SS58 addresses to 0x{hex} format.
...
pub fn account_to_bytes<AccountId: Encode>(acc: &AccountId) -> [u8; AccountAddress::LENGTH] {
	...
	let skip = if bytes.len() < LENGTH { LENGTH - bytes.len() } else { 0 };

	(&mut result[skip..]).copy_from_slice(&bytes);

	trace!("converted: (with skip: {})\n\t{:?}\n\tto {:?}", skip, bytes, result);
...
#[cfg(test)]
mod tests {
	use super::{account_to_account_address, address_to_account, AccountAddress};
