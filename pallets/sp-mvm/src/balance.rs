...
//! PalletId using to deposit/withdraw tokens to/from current pallet, it solves issue with total
//! issuance.
//!
...
use sp_std::{cmp::PartialEq, default::Default, prelude::*, vec::Vec};
	...
	BalancesAdapter<AccountId, Currencies, CurrencyId>
{
	...
	pub fn new(pallet_id: PalletId) -> Self {
		Self { pallet_id, _phantom: core::marker::PhantomData }
	}
		...
		match currency_id {
			Ok(id) => {
				...
				address_to_account::<AccountId>(address)
					...
					.and_then(|address| {
						Currencies::reducible_balance(id, &address, false).try_into().map_err(
							|_err| error!("can't convert native balance to VM balance type."),
						)
					})
					.ok()
			},
...
pub mod boxed {
	...
	impl<
			...
			CurrencyId: FullCodec
				...
				+ Default
				+ 'static,
		> From<super::BalancesAdapter<AccountId, Currencies, CurrencyId>> for BalancesBoxedAdapter
	{
