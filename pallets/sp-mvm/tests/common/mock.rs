...
use frame_support::{
	parameter_types,
	traits::{ConstU32, Everything, OnFinalize, OnInitialize, WithdrawReasons},
...
use sp_runtime::{
	testing::Header,
	traits::{BlakeTwo256, ConvertInto, IdentityLookup, Lazy, Verify},
};
use sp_std::{convert::TryFrom, fmt::Debug, vec};
...
// --- gas --- //
/// By inheritance from Moonbeam and from Dfinance (based on validators statistic), we believe max
/// 4125000 gas is currently enough for block. In the same time we use same 500ms Weight as Max
/// Block Weight, from which 75% only are used for transactions. So our max gas is GAS_PER_SECOND *
/// 0.500 * 0.65 => 4125000.
