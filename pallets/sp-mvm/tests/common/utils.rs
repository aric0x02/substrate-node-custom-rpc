...
use move_vm::{io::state::State, types::ModulePackage};
/// Utils functions to work with Move VM.
use std::convert::TryFrom;

use sp_mvm::storage::MoveVmStorage;

use super::{addr::*, assets::*, mock::*};
...
pub fn publish_package(signer: AccountId, package: &Package, gas_limit: Option<u64>) -> PsResult {
	...
	check_storage_package(to_move_addr(signer), package.bytes().to_vec(), package.modules());
	Ok(result)
}
...
pub fn publish_package_as_root(package: &Package, gas_limit: Option<u64>) -> PsResult {
	...
	check_storage_package(CORE_CODE_ADDRESS, package.bytes().to_vec(), package.modules());
	Ok(result)
}
...
pub fn check_storage_module<Bc: AsRef<[u8]>>(account_address: AccountAddress, bc: Bc, name: &str) {
	...
	T: std::cmp::PartialEq + std::fmt::Debug,
{
	...
	let stored: T = bcs::from_bytes(&blob).expect(&format!("Resource '{}' should exists", tt_str));
