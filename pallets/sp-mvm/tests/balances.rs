...
use frame_support::{
	assert_err_ignore_postinfo, assert_ok, dispatch::DispatchError, traits::VestingSchedule,
};
use move_core_types::{
...
where
	T: Into<u128>,
{
	let expected = StoreU128 { val: expected.into() };
	let tag = StructTag {
		address,
...
where
	T: Into<u64>,
{
	let expected = StoreU64 { val: expected.into() };
	let tag = StructTag {
		address,
		module: Identifier::new(modules::user::STORE.name()).unwrap(),
...
fn execute_get_token_balance() {
	RuntimeBuilder::new()
		...
		.execute_with(|| {
			...
			// Deposit KSM tokens.
			assert_ok!(orml_tokens::Pallet::<Test>::deposit(currency, &account, to_deposit));
...
fn execute_token_transfer() {
	RuntimeBuilder::new()
		...
		.execute_with(|| {
			...
			assert_ok!(orml_tokens::Pallet::<Test>::deposit(currency, &bob, to_deposit));
...
fn check_token_total_supply() {
	RuntimeBuilder::new()
		...
		.execute_with(|| {
			...
			assert_ok!(orml_tokens::Pallet::<Test>::deposit(currency, &bob, to_transfer));
...
mod adapter {
	...
	use sp_mvm::balance::{boxed::BalancesAdapter as BoxedBalancesAdapter, BalancesAdapter};
	...
	fn test_get_token_balance_with<T: BalanceAccess>(adapter: &T) {
		RuntimeBuilder::new()
			...
			.execute_with(|| {
				...
				assert_ok!(orml_tokens::Pallet::<Test>::deposit(currency, &origin, to_deposit));
	...
	#[test]
	fn get_balance() {
		let adapter = BalancesAdapter::<AccountId, Currencies, CurrencyId>::new(MVMPalletId::get());
	...
	fn get_balance_boxed() {
		let adapter =
			BoxedBalancesAdapter::from(BalancesAdapter::<AccountId, Currencies, CurrencyId>::new(
				MVMPalletId::get(),
			));
		test_get_balance_with(&adapter);
	}
