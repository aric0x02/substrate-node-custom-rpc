...
use mock::{
	...
	ExtBuilder, NativeCurrency, PalletBalances, RuntimeOrigin, System, Test, Tokens, DOT, ID_1,
...
fn call_event_should_work() {
	ExtBuilder::default().one_hundred_for_alice_n_bob().build().execute_with(|| {
		...
		assert_eq!(Currencies::free_balance(X_TOKEN_ID, &alice()), 50);
		assert_eq!(Currencies::free_balance(X_TOKEN_ID, &bob()), 150);
		System::assert_last_event(crate::Event::Transferred(X_TOKEN_ID, alice(), bob(), 50).into());
		...
		System::assert_last_event(crate::Event::Transferred(X_TOKEN_ID, alice(), bob(), 10).into());
		...
		System::assert_last_event(crate::Event::Deposited(X_TOKEN_ID, alice(), 100).into());
		...
		System::assert_last_event(crate::Event::Withdrawn(X_TOKEN_ID, alice(), 20).into());
	});
}
...
fn sweep_dust_tokens_works() {
	ExtBuilder::default().build().execute_with(|| {
		orml_tokens::Accounts::<Test>::insert(
			bob(),
			DOT,
			orml_tokens::AccountData { free: 1, frozen: 0, reserved: 0 },
		);
		orml_tokens::Accounts::<Test>::insert(
			eva(),
			DOT,
			orml_tokens::AccountData { free: 2, frozen: 0, reserved: 0 },
		);
		orml_tokens::Accounts::<Test>::insert(
			alice(),
			DOT,
			orml_tokens::AccountData { free: 0, frozen: 1, reserved: 0 },
		);
		orml_tokens::Accounts::<Test>::insert(
			DustAccount::get(),
			DOT,
			orml_tokens::AccountData { free: 100, frozen: 0, reserved: 0 },
		);
		...
		assert_eq!(orml_tokens::Accounts::<Test>::contains_key(bob(), DOT), false);
...
fn sweep_dust_native_currency_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		System::assert_last_event(crate::Event::DustSwept(NATIVE_CURRENCY_ID, bob(), 1).into());
